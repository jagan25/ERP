...
echo "$edate";
 	...
 	$sql="SELECT SUM('$type') FROM case3 where p_name='$product' and date between '$sdate' and '$edate' group by date";
...
$sql="SELECT sum($type) as type FROM case3 where p_name='$product' and date between '$sdate' and '$edate' group by date";
$qury=mysql_query($sql);
while($row = mysql_fetch_assoc($qury))
{
 $value[$j] = "{$row['type']}";
...
$r=1;
$sql="SELECT distinct date FROM case3 where p_name='$product' and date between '$sdate' and '$edate'";
$qury=mysql_query($sql);
while($row = mysql_fetch_assoc($qury))
{
 $key[$r] = "{$row['date']}";
